...
using Bit2Sky.Bit2EHR.MultiTenancy.Demo;
    ...
    IEventHandler<RecurringPaymentSucceedEventData>
{
    ...
    )
    {
        ...
        if (isInTrialPeriod && !subscriptionEndDate.HasValue)
        {
            throw new UserFriendlyException(LocalizationManager.GetString(
                Bit2EHRConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
        }
    ...
    public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
    {
        await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            var error = LocalizationManager.GetSource(Bit2EHRConsts.LocalizationSourceName)
    ...
    public void HandleEvent(RecurringPaymentSucceedEventData eventData)
    {
        _unitOfWorkManager.WithUnitOfWork(() =>
        {
            ...
            if (editionItem == null || !editionItem.Product.Equals(Bit2EHRConsts.ProductName))
            {
                Logger.Warn(
                    $"This is not a payment for {Bit2EHRConsts.ProductName} for edition subscription cycle! " +
