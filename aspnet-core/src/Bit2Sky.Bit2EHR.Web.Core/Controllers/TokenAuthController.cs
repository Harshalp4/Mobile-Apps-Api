...
using Bit2Sky.Bit2EHR.Web.Authentication.JwtBearer;
using Bit2Sky.Bit2EHR.Web.Authentication.TwoFactor;
using Bit2Sky.Bit2EHR.Web.Models.TokenAuth;
...
using Bit2Sky.Bit2EHR.Net.Sms;
using Bit2Sky.Bit2EHR.Notifications;
using Bit2Sky.Bit2EHR.Security.Recaptcha;
using Bit2Sky.Bit2EHR.Web.Authentication.External;
...
public class TokenAuthController : Bit2EHRControllerBase
{
    ...
    private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
    {
        ...
        var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
            .ToString(Bit2EHRConsts.DateTimeOffsetFormat));
